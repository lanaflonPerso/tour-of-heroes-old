...
public class HeroService
{
	...
	public Promise<Hero> post( Hero hero )
	{
		...
		return http.post( heroesUrl, JSON.stringify( hero ), new JsObject().set( "headers", headers ) )
				.toPromise()
				.<Hero> then( response -> response.json() )
				.onCatch( this::handleError );
	}
	...
	public Promise<Hero> put( Hero hero )
	{
		...
		return http.put( url, JSON.stringify( hero ), new JsObject().set( "headers", headers ) )
				.toPromise()
				.<Hero> then( response -> response.json() )
				.onCatch( this::handleError );
	}
	...
	{
		return http.delete( heroesUrl + "/" + hero.id )
				.toPromise()
				.onCatch( this::handleError );
	}
